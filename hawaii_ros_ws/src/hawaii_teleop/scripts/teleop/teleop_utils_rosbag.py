...
from rosbag2_py import SequentialWriter, SequentialReader, StorageOptions, ConverterOptions, RecordOptions
...
class ImageRecorder(Node):
    ...
    def setup_rosbag_writer(self):
        storage_options = StorageOptions(uri='image_recordings', storage_id='sqlite3')
        converter_options = ConverterOptions('', 'cdr')
        record_options = RecordOptions()
        record_options.all = False
        record_options.topics = [f"/usb_{cam}/image_raw" for cam in self.camera_names]
        bag_writer = SequentialWriter()
        bag_writer.open(storage_options, converter_options)
        bag_writer.create_topic('/usb_cam_high/image_raw', 'sensor_msgs/msg/Image', 'cdr', '')
        bag_writer.create_topic('/usb_cam_front/image_raw', 'sensor_msgs/msg/Image', 'cdr', '')
        bag_writer.create_topic('/usb_cam_left/image_raw', 'sensor_msgs/msg/Image', 'cdr', '')
        bag_writer.create_topic('/usb_cam_right/image_raw', 'sensor_msgs/msg/Image', 'cdr', '')
        return bag_writer
    ...
    def image_cb_cam_high(self, data):
        ...
        self.bag_writer.write('/usb_cam_high/image_raw', data, self.get_clock().now().to_msg())
        return
    
    def image_cb_cam_front(self, data):
        self.cam_front_image = data
        self.bag_writer.write('/usb_cam_front/image_raw', data, self.get_clock().now().to_msg())
        return
    
    def image_cb_cam_left(self, data):
        self.cam_left_image = data
        self.bag_writer.write('/usb_cam_left/image_raw', data, self.get_clock().now().to_msg())
        return
    
    def image_cb_cam_right(self, data):
        self.cam_right_image = data
        self.bag_writer.write('/usb_cam_right/image_raw', data, self.get_clock().now().to_msg())
        return
...
def pull_images_from_bag(bag_file_path, data_dict):
    ...
    while reader.has_next():
        ...
        if topic in [f'/usb_{cam_name}/image_raw' for cam_name in camera_names]:
            cam_name = topic.split('_')[-2]
            try:
